    ...
    function threadedComments($comments, $options) {
        ...
        if ($comments->authorId) {
            if ($comments->authorId == $comments->ownerId) {
                ...
                $Identity = '<label class="label bg-dark m-l-xs" style="margin-left:18px">'._mt("作者").'</label>';
            } else {
            ...
            <div id="<?php $this->respondId(); ?>" class="respond comment-respond no-borders">
                ...
                <form id="comment_form" method="post" action="<?php $this->commentUrl() ?>"  class="comment-form" role="form">
                    <div class="comment-form-comment form-group">
                        ...
                        <textarea id="comment" class="textarea form-control OwO-textarea" name="text" rows="5" placeholder="<?php _me("博客开通了CDN加速，所以评论内容不会马上显示，提交后请一顿操作：Ctrl+F5，不谢！") ?>" onkeydown="if(event.ctrlKey&&event.keyCode==13){document.getElementById('submit').click();return false};"><?php $this->remember('text'); ?></textarea>
