					...
					if (@$_GET['action'] == 'open_world') {
						if (!empty($_GET['password'])) {
							...
							if (Utils::encodeData($password) == $md5) {
								echo 1;
								if ($type == 'index') {
									Typecho_Cookie::set('open_new_world', Utils::encodeData($password));
								} elseif ($type == 'category') {
									$category = $_GET['category'];
									Typecho_Cookie::set('category_' . $category, Utils::encodeData($password));
								}
							} else {
								echo -1;
							}
